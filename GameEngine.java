...
public GameEngine(){
        super("Fleet Destroyer Engine - User: " + Player.getCurrentUser());
}

//////HADIIIIII
public static void setButtonColor(int location){
	buttonF[location].setBackground(Color.BLACK);
		...
		FinalizeShips.addActionListener(new ActionListener(){
				...
				//	Updater.setme();
					...
					if (FullSizeAC == true && FullSizeBS == true && FullSizeSUB == true 
							...
							textField.setVisible(true);//edited by hadi
...
/*
public static void setShotHit(String hit, int hitmarker){
	if(hit.equals("Yes")){
		buttonF[hitmarker].setBackground(Color.GREEN);
	}
	if(hit.equals("No")){
		buttonF[hitmarker].setBackground(Color.BLACK);
	}
...
public static void recieveandprint(String hi){
	System.out.println(hi);
}

public void setFireBoard(Integer a, Integer b){
	if(a == 0){
	buttonF[b].setBackground(Color.WHITE);
	}if(a == 1){
		buttonF[b].setBackground(Color.RED);
	}
}
